    ...
    while True:
        ...
        try:
            ...
            with open('results/'+ENV+'/results_noise_test.pickle', 'wb+') as f:
                pickle.dump([test_regular, test_LRRL, test_regular_noise_a, test_regular_noise_b, test_LRRL_noise_a, test_LRRL_noise_b], f)
            ...
            worst_regular = np.ndarray.min(history, axis=0)
            ...
            worst_LRRL = np.ndarray.min(history, axis=0)

            # Testing EDI for different zetas
            zeta = [1, 1.2, 1.4, 1.6, 1.8, 2.0, 2.2, 2.4, 2.6, 2.8]
            for z in zeta:
                ...
                worst_regular = np.vstack((worst_regular, np.ndarray.min(history, axis=0)))
                ...
                worst_LRRL = np.vstack((worst_LRRL, np.ndarray.min(history, axis=0)))
